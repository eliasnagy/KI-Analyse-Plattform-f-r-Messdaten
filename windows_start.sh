...
case $hauptwahl in
  1)
    ...
    echo "a) Training"
    echo "b) Live-Modus"
    read -p "Wähle Unteroption [a-c]: " unterwahl
    case $unterwahl in
      a)
        ./env_classic/Scripts/python.exe ./klassische_modelle/train.py --model $MODEL_TYPE
        ;;
      b)
        ./env_classic/Scripts/python.exe ./klassische_modelle/live.py --model $MODEL_TYPE
        ;;
  ...
  3)
    ...
    case $unterwahl in
      a)
        ./env_torch/Scripts/python.exe ./torch/train.py
        ;;
      b)
        ./env_torch/Scripts/python.exe ./torch/inference.py
        ;;
      *) echo "Ungültige Unteroption.";;
    esac
    ;;
