...
arr = np.load("c1_features_X.npy", allow_pickle=False)
...
arr = np.load("c1_features_X.npy")
for name, val in zip(feat_names, arr[0]):
    print(f"{name}: {val}")
"""

import numpy as np
X = np.load("c1_features_X.npy"); print(X.shape)
y = np.load("c1_features_y.npy"); print(y.shape)
print("erste y:", y[:5])